...
func TestCoin_Spawn(t *testing.T) {
	...
	ca := service.InstanceID{service.ZeroDarc, service.NewSubID(inst.Hash())}
...
func TeContractCoin_InvokeTransfer(t *testing.T) {
	// Test that a coin can be transferred
	ct := newCT()
	coAddr1 := service.InstanceID{service.ZeroDarc, service.ZeroSubID}
	coAddr2 := service.InstanceID{service.ZeroDarc, service.ZeroSubID}
	...
	inst := service.Instruction{
		...
		Invoke: &service.Invoke{
			Command: "transfer",
			Args: service.NewArguments(service.Argument{"coins", coinOne},
				service.Argument{"destination", coAddr1.Slice()}),
		},
	}
	sc, co, err := ContractCoin(ct, inst, []service.Coin{})
	...
	inst = service.Instruction{
		...
		Invoke: &service.Invoke{
			Command: "transfer",
			Args: service.NewArguments(service.Argument{"coins", coinOne},
				service.Argument{"destination", coAddr2.Slice()}),
		},
	}
	sc, co, err = ContractCoin(ct, inst, []service.Coin{})
	require.Nil(t, err)
