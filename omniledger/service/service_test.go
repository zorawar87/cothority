...
// because we the config contract fails).
func TestService_ViewChange(t *testing.T) {
	testViewChange(t, 5, 1)
}
...
func testViewChange(t *testing.T, nHosts, nFailures int) {
	...
	// The first nFailures hosts then the next node at index nFailures
	// should take over.
	...
	for i := 0; i < 5; i++ {
		...
		if config.Roster.List[0].Equal(s.services[nFailures].ServerIdentity()) {
			ok = true
			break
		}
	}
